...
function sobeks_posts_in_category() {
	...
	if ($num_of_args != 0) {
		...
		if($limit_it != 0) { $limit_text = ' LIMIT 0, ' . $limit_it; } else { $limit_text = ''; }
		$list_of_args = explode(",",$list_of_args);
		$list_of_args = array_diff($list_of_args, array('', 'commentsort', 'date', 'count', 'comments', 'author', 'orderlist', 'unorderlist', 'commalist'));
