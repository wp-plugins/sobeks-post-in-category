...
function sobeks_posts_in_category() {
	...
	if ($number_of_args != 0) {
		...
		for ($i = 0; $i < $number_of_args ; $i++) {
			...
			if($list_of_args[$i] == "comments") { $display_comment_count = 1; }
	...
	$posts_in_term = $wpdb->get_results("SELECT wp_posts.ID, wp_posts.post_date, wp_posts.post_title, wp_posts.post_status, wp_posts.comment_count, wp_term_relationships.object_id, wp_term_relationships.term_taxonomy_id FROM wp_posts, wp_term_relationships, wp_term_taxonomy WHERE wp_posts.ID = wp_term_relationships.object_id AND wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id AND wp_term_taxonomy.taxonomy = 'category' AND wp_posts.post_status = 'publish' " . $the_categories_result . " AND wp_posts.post_date < NOW( ) ORDER BY " . $order_posts_by);
